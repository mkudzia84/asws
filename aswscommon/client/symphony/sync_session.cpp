...
void sync_session::open(session_attributes& attr)
{
	format::scoped_prefix sp("[sync_session::open]");
	...
	psout << "Creating new session SYNC : { name : " << attr.name << ", type : " << attr.type 
	...
	int sym_flags = soam::Session::ReceiveSync;
...
void sync_session::close(bool wait_for_results)
{
...
void sync_session::submit_segment_start()
{
	std::stringstream prefix;
...
void sync_session::submit_segment_end()
{
	std::stringstream prefix;
	prefix << "[session-" << _name << "::submit]";
	format::scoped_prefix sp(prefix.str());
...
void sync_session::submit(asws::workload::iworkload_ptr workload)
{
	std::stringstream prefix;
	prefix << "[session-" << _name << "::submit]";
...
void sync_session::wait()
{
	...
	psout << "Processing outputs of the tasks, processing 100 at a time" << std::endl;

	int n_outstanding = 0;
	while ((n_outstanding = _bookkeeper.get_tasks_outstanding()) != 0) {


		int n_fetch = n_outstanding;
		if (n_fetch > 100)
			n_fetch = 100;
		
		soam::EnumItemsPtr task_iter = _session->fetchTaskOutput(n_fetch);
		soam::TaskOutputHandlePtr task_handle;
		while (task_iter->getNext(task_handle)) {
			std::stringstream output_prefix;
			output_prefix << "[task:" << task_handle->getId() << "][output]";
			format::scoped_prefix orp(output_prefix.str());
